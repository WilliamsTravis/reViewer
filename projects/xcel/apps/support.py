...
DATASETS = {
    "liberal": pd.read_csv(DATAPATH.join("liberal_closest_connection.csv")),
    "moderate": pd.read_csv(DATAPATH.join("moderate_closest_connection.csv")),
    "conservative": pd.read_csv(DATAPATH.join("conservative_closest_connection.csv"))
    }
...
def fix_cfs(files):
    ...
    for f in files:
        df = pd.read_csv(f)
        if "closest" in f and "mean_cf" in df.columns:
            if df["mean_cf"].max() < 1:
...
# Chart functions
def get_ccap(paths, y, mapsel, point_size, reset, trig):
...
def get_scatter(paths, x, y, mapsel, point_size, reset, trig):
...
def get_histogram(paths, y, mapsel, point_size, reset, trig):
    """Return a histogram."""
    df = None
    for key, path in paths.items():
        if df is None:
            ...
            df = df[["gid", y]]
            df["land_use"] = key
            df = df[["gid", y, "land_use"]]
        else:
            df2 = DATASETS[path].copy()
            df2["gid"] = df2.index
            df2 = df2[["gid", y]]
            df2["land_use"] = key
            df2 = df2[["gid",  y, "land_use"]]
            df = pd.concat([df, df2])
...
def get_boxplot(paths, y, mapsel, point_size, reset, trig):
    ...
    fig = px.box(df, x="land_use", y=y, color="land_use", labels={y: UNITS[y]})
