...
from apps import main_page, config_page, test_page
...
def change_page(pathname):
    ...
    test_style = BUTTON_STYLES["on"]
    if pathname == "/apps/config_page":
        page = config_page.layout
        app_style = BUTTON_STYLES["off"]
        test_style = BUTTON_STYLES["off"]
    elif pathname == "/apps/test_page":
        page = test_page.layout
        app_style = BUTTON_STYLES["off"]
        config_style = BUTTON_STYLES["off"]
    else:
        page = main_page.layout
        test_style = BUTTON_STYLES["off"]
        config_style = BUTTON_STYLES["off"]
    return page, app_style, config_style, test_style
