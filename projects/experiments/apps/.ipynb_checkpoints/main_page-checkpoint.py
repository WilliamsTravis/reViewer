...
layout = html.Div(
    children=[
        ...
        # Synced variable options
        html.Div(
               id="shared_variable_options_div",
               children=[
                   html.H5("Variable"),
                   dcc.Dropdown(
                       id="shared_variable_options",
                       clearable=False,
                       options=VARIABLES,
                       multi=False,
                       value="mean_cf"
...
@app.callback([Output('map_options_tab', 'children')],
              [Input("sync_variables", "n_clicks")])
def map_sync_tab_options(sync):

    vstyle = {'height': '25px', 'padding': '0'}
    if sync % 2 == 1:
        vstyle = {"display": "none"}
...
@app.callback([Output('chart_options_tab', 'children'),
              ...
              [Input('chart_options_tab', 'value'),
               ...
               Input("sync_variables", "n_clicks")])
def chart_tab_options(tab_choice, chart_choice, sync):
...
@app.callback([Output('hubheight_options_div', 'style'),
              ...
              [Input('map_options_tab', 'value'),
               Input("sync_variables", "n_clicks")])
def map_tab_options(tab_choice, sync):
...
@app.callback([Output("sync_variables", "children"),
               ...
               Output("shared_variable_options_div", "style")],
              [Input("sync_variables", "n_clicks")])
def toggle_sync_buttons(click):
    ...
    if click % 2 == 1:
        ...
        style = BUTTON_STYLES["on"]
        var_bttn_style = {"width": "25%", "margin-bottom": "50px"}
    else:
        children = "Sync Variables: Off"
        style = BUTTON_STYLES["off"]
        var_bttn_style = {"display": "none"}

    return children, style, var_bttn_style
